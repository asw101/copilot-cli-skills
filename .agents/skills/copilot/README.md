...
| **Reasoning effort levels** | `low/medium/high/xhigh` (full range) | `low/medium/high` (no xhigh) | `low/medium/high` (no xhigh in this binary's API) |
...
```
[exit 0 · cli/claude-opus-4.7 · premium 7.5 · in/out: 0/6 · 4.1s]
[exit 0 · acp/claude-opus-4.7 · 7.2s]
[exit 0 · sdk/claude-opus-4.7 · premium 7.50 · in/out: 24222/6 · cache: 11280↑/12936↓ · 4.6s]
```
...
- **Effort cap divergence.** CLI flag accepts `xhigh`; ACP and SDK SessionConfig cap at `high`. The CLI runner has a `THINKING_CAPABLE` model gate so non-thinking models don't crash.
